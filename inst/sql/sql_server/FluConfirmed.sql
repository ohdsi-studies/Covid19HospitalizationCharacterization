...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4266367)and invalid_reason is null
...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (2213181,3043891,3045856,3044141)and invalid_reason is null
...
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1799139)and invalid_reason is null
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1799139)
  and c.invalid_reason is null
...
select event_id, person_id,
  case when DATEADD(day,180,start_date) > start_date then DATEADD(day,180,start_date) else start_date end as end_date
INTO #strategy_ends
